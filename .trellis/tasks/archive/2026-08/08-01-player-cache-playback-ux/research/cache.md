...
| `core/data/.../api/TrimMusicApi.kt` | OkHttp endpoint/request execution and image byte downloads. |
...
| `TrimMusicApiTest.kt` | Request/auth contract tests; no HTTP or repository caching tests. |
...
3. `SessionRepository.connect` constructs a session-scoped `TrimMusicApi`/`OkHttpClient` (`SessionRepository.kt:154-159`). The client disables redirects and transport retry, and configures no OkHttp disk cache (`TrimMusicApi.kt:196-201`). API calls are synchronous OkHttp executions on `Dispatchers.IO` (`:122-174`).
