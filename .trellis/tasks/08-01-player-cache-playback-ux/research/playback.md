...
- Scope: mixed (TV client source, Trim Music v0.9.16 server snapshot, local Feiniu Music mobile APK, official Android Media3 documentation)
- Date: 2026-08-01
...
TrackCollection / BrowseHome
  ...
  -> ExoPlayer -> HTTP CacheDataSource -> Trim Music direct stream
```
...
1. `roam-start` returns `current` and an already-prefetched `next`; the client holds both in a `RoamWindow` (`core/data/src/main/kotlin/com/fnmusic/tv/core/data/repository/MusicRepository.kt:159-164`; server behavior at `.detail/trim-music-v0.9.16/core/service/roam.go:36-80`).
...
- Server roam calls a track playable when it is accessible/present; it does not exclude CUE (`.detail/trim-music-v0.9.16/core/service/roam.go:217-245,335-340`). A CUE track can therefore be `current` or `next`.
...
- The local Trim Music v0.9.16 snapshot is server source, not a player implementation. It confirms roam is a device-scoped linked window in memory, with `roamId` required as the next/previous relative cursor (`docs/API.md:380-388,731-737`; `.detail/trim-music-v0.9.16/core/service/roam.go:83-201`). Sessions expire after 24 hours and are lost on server restart (`.detail/trim-music-v0.9.16/core/service/roam.go:20-33,419-554`).
...
- `core/data/src/main/kotlin/com/fnmusic/tv/core/data/api/TrimMusicApi.kt` - roam endpoints and direct stream URL construction.
...
- `.detail/trim-music-v0.9.16/core/service/roam.go` - authoritative server roam linked-window/session behavior.
